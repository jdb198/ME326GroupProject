...
class PerceptionNode(Node):
    def __init__(self):
        ...
        self.need_world_coordinate = True # Always False for Locobot 3, default 1 for Locobot 1
        self.image_path = None
        self.arm_base_coords = None
        self.called_YOLO = False
        self.published_arm_base_coord = False
    ...
    def text_callback(self, msg):
        if self.called_YOLO and self.published_arm_base_coord:
            self.publish_target_coord(self.arm_base_coords)
            self.published_arm_base_coord = False
            return
    ...
    def process_if_ready(self):
        ...
        if not self.target_pixel or self.called_YOLO:
            return  # Skip processing if there is no target pixel to transform
        if not all([self.latest_rgb, self.latest_depth, self.rgb_camera_info, self.latest_odom]):
    ...
    def process_image(self, rgb_msg, depth_msg, camera_info_msg, odom_msg):
        ...
        if self.need_world_coordinate:
            ...
            self.arm_base_coords = self.camera_to_arm_base(camera_coords, camera_to_arm_base)
            ...
            self.arm_base_coords = arm_base_coords
        ...
        self.target_pixel = None
        self.image_path = None
        print("Cleared self.target_pixel and image_path")
